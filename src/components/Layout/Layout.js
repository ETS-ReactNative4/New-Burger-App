  ...
  render(){
    return(
       <Scrollbars style={{ width: '100vw', height: '100vh'}} renderThumbVertical={({ style,...props }) =>
            <div {...props} style={{ ...style, backgroundColor: 'gray',width:'7',opacity: '0.8',cursor:'pointer',margin:'0'}}/>
        }>
      <div>
        <HamburgerModal on={this.state.hamburgerToggle} toggle={this.hamburgerToggle}/>
        ...
        <main>
          {this.props.children}
          {this.props.location.pathname === '/'&& (Object.keys(this.props.priceList).length>0 ? <Footer /> :null)}
        </main>
      </div>
      </Scrollbars>
